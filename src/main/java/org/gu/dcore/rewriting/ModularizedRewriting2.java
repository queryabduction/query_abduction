...
public class ModularizedRewriting2 {
	...
	public List<Rule> rewrite() {
		...
		while (!rewQueue.isEmpty()) {
			...
			if (!selected.add(r.getRuleIndex()))
				continue;
			...
			for (Atom a : r.getNormalAtoms()) {
				if (a.getPredicate().getName().equals("ANS"))
					continue;
	 ...
	 */
	private void rewriteBlock(BlockRule blockRule, Block block, boolean restricted, List<Rule> result,
			Queue<BlockRule> rewQueue) {
		...
		while (!bqueue.isEmpty()) {
			...
			queue.add(new Tuple6<>(blockAtom.getTerms(), br.getBody(), na, new AtomSet(), new AtomSet(),
					new HashMap<>()));
			...
			while (!queue.isEmpty()) {
				...
				for (BlockRule hr : rs) {
					...
					// List<AggregateUnifier> unifiers = Unify.getAggregatedPieceUnifier(t.c, t.b,
					// hr, restricted_var);
					...
					if (!unifiers.isEmpty()) {
						...
						if (!ex_rew && selected.add(hr.getRuleIndex())) {
							if (hr.getMblocks().isEmpty())
								result.add(hr);
							else {
								Rule n_rule = RuleFactory.instance().createRule(hr.getHead(), next_target, tails);
								result.add(n_rule);
							}
						}
						...
						for (SinglePieceUnifier u : unifiers) {
							...
							if (!ex_rew) {
								...
								if (used_rules != null && used_rules.contains(hr.getRuleIndex()))
									continue;
							}
							...
							for (int i = 0; i < t.a.size(); i++) {
								Term _t = t.a.get(i);
								if (eliminated.contains(_t))
									rw_t.add(blank);
								else
									rw_t.add(u.getImageOf(_t, 0));
							}
							...
							if (!next_target.isEmpty()) {
								...
								if (ex_rew) {
									...
									if (trace != null)
										used_rules = new HashSet<>(trace);
									else
										used_rules = new HashSet<>();
								...
								queue.add(new Tuple6<>(rw_t, o_rewriting, rewriting, uc, rewrited_target,
										trace_used_nonex_rules));
							}
						}
					}
				}
