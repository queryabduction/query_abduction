...
//	}
	...
	public void testApp10()
	{
		...
		Program P = parser.parse("t(X1, Y1, Y2) :- r(X1)." +
				"u(X1, X2, Y1) :- t(X1, X2, X3)." +
				"p(X1), v(X1, X2) :- u(X1, X2, X3)." +
				"m(X1) :- t(X1, X2, X3), v(X1, X2), s(X1).");

		Modularizor modularizor = new Modularizor(P.getRuleSet());
		modularizor.modularize();
		modularizor.getMarking().printMarked();

		ConjunctiveQuery query = new QueryParser().parse("?(X) :- m(X).");
